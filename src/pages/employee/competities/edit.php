...
if(!empty($_POST)){
    ...
    $winner = User::getByName($_POST["winner"]);
...
<div class="d-flex justify-content-center mt-5 text-dark">
        <h2>Game <snap class="text-info">Veranderen</snap></h2>
</div>
    ...
    <div class="d-flex align-items-center justify-content-center">
        <form style="width: 35%;" class="form-horizontal p-5 rounded" action="" method="POST">
            ...
            <div class="mb-3 d-flex align-items-center justify-content-between">
                <input type="text" name="whitePlayer" list="customers" class="form-control fs-4" value="<?=User::get($game->getWhiteUserId())->getName()?>" required>
            ...
            <div class="mb-3 d-flex align-items-center justify-content-between">
                <input type="text" name="blackPlayer" list="customers" class="form-control fs-4"  value="<?=User::get($game->getBlackUserId())->getName()?>" required>
                <datalist id="customers">
                    <?php foreach (User::getAllMembers() as $member) : ?>
                        <option class="form-control fs-4" value="<?= $member["name"] ?>"></option>
                    <?php endforeach; ?>
            ...
            <div class="mb-3">
                ...
                <input type="datetime-local" class="form-control fs-4" name="startDate" value="<?=$game->getStartTime()?>" required>
            </div>
            <div class="mb-3">
                <label class="form-label">Eind datum/tijd</label>
                <input type="datetime-local" class="form-control fs-4" value="<?=$game->getEndTime()?>" name="endDate">
            </div>
